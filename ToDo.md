...
## 内容
- 照度ロバストな画像特徴量を用いたセマンティックセグメンテーション  
    一般交通シーンでは、環境によって大きく照度が変動する。そのため、
    大量の教師データが必要となり、データの収集コストが問題となっている。  
    照度変化による入力データのばらつきを抑える画像特徴量抽出層を導入した。  
    LBPをベースにして、注目画素と周辺画素をピクセル単位で比較するのではなく、領域での比較を行う。  
    LBPでは、ピクセル単位での比較であるため、ノイズが乗ると被写体本来のピクセル間関係性が壊れることがある。
    そのため、領域での比較を行うことでピクセル平均化によりノイズを低減、複数画素の比較によるレベル方向へのダイナミックレンジ拡張が期待できる。  
    ピクセル単位ではなく、領域単位で処理されるため、実質的な空間解像度ではLBPに劣るが、ロバスト性向上を実現する。  
    セマセグ用のエンコーダ、デコーダのネットワークに対して、そのままの学習と、特徴量抽出層を付け加えたものとでの比較実験  
    照度変化に対しては非常に強力である。cityscapes画像ではノイズやブラーに対して弱かったが、
    昼夜データセットでは、カメラ画質がノイズを含んだり、ブラーがかかったりするため、学習データによってロバスト性を獲得できている。  
    昼と夜の量データを学習させたモデルとの比較では認識精度が劣っているため、昼と夜とでコンテキストが異なる被写体への対応を検討する。

    
- LBP  
    画像の一部分を切り出し、さらにそこから3*3の範囲に注目する。
    注目画素の周囲8ピクセルを、注目画素Pと比較し、
    Pよりも明るければ1、そうでなければ0として、1塊のバイナリデータにする。  
    これによって、画像は256(8ビット)階調グレースケールに変換され、
    また、同じ画像で明るいものと暗いものをLBP画像に変換すると、ほぼ同じ画像が出来上がる(照明変化に強い)
    Aの画像とBの画像の類似度を検索することも可能(類似画像検索)

- SqueezeNet  
    DNNモデルを性能を落とさずにコストの削減をしたもの。  
